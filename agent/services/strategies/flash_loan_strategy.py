"""
Flash loan attack vector detection strategy.
"""
import json
import logging
...
from agent.services.prompts.flash_loan_prompt import FLASH_LOAN_PROMPT
...
class FlashLoanStrategy(BaseStrategy):
    """
    Specialized strategy for detecting flash loan exploitable vulnerabilities.
    
    Focuses on identifying price manipulation, economic logic flaws, and
    operations vulnerable to flash loan attacks.
    """
    ...
    def analyze(
        ...
        """
        Execute flash loan vulnerability analysis.
        ...
        try:
            ...
            # Prepare the flash loan prompt with all information
            audit_prompt = FLASH_LOAN_PROMPT.format(
                contracts=contracts,
                docs=docs,
            ...
            logger.info(f"[{self.name}] Sending flash loan analysis request to OpenAI")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert Solidity smart contract auditor specializing in flash loan attack vectors."},
