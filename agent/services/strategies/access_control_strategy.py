"""
Access control and privilege escalation detection strategy.
"""
import json
import logging
...
from agent.services.prompts.access_control_prompt import ACCESS_CONTROL_PROMPT
...
class AccessControlStrategy(BaseStrategy):
    """
    Specialized strategy for detecting access control vulnerabilities.
    
    Focuses on identifying missing access controls, privilege escalation
    opportunities, and authorization bypasses.
    """
    
    def __init__(self, api_key: str, model: str):
        """
        Initialize the access control strategy.
    ...
    def analyze(
        ...
        """
        Execute access control vulnerability analysis.
        ...
        try:
            ...
            # Prepare the access control prompt with all information
            audit_prompt = ACCESS_CONTROL_PROMPT.format(
                contracts=contracts,
                docs=docs,
            ...
            logger.info(f"[{self.name}] Sending access control analysis request to OpenAI")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert Solidity smart contract auditor specializing in access control and privilege escalation vulnerabilities."},
